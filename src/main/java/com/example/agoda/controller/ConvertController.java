...
public class ConvertController {
    ...
    private static final List<CidEntry> STATIC_CIDS = List.of(
        new CidEntry("구글 지도 1", 1833982),  new CidEntry("구글 지도 2", 1917614),
        new CidEntry("구글 지도 3", 1829668),  new CidEntry("구글 검색 1", 1908617),
        new CidEntry("구글 검색 2", 1921868),  new CidEntry("구글 검색 3", 1922847),
        new CidEntry("네이버", 1881505),      new CidEntry("Bing", 1911217),
        new CidEntry("다음", 1908762),        new CidEntry("DuckDuckGo", 1895204),
        new CidEntry("국민카드", 1563295),     new CidEntry("우리카드", 1654104),
        new CidEntry("우리카드(마스터)", 1932810), new CidEntry("현대카드", 1768446),
        new CidEntry("BC카드", 1748498),      new CidEntry("신한카드", 1760133),
        new CidEntry("신한카드(마스터)", 1917257), new CidEntry("토스", 1917334),
        new CidEntry("하나카드", 1729471),     new CidEntry("카카오페이", 1845109),
        new CidEntry("마스터카드", 1889572),   new CidEntry("유니온페이", 1801110),
        new CidEntry("비자", 1889319),        new CidEntry("대한항공(적립)", 1904827),
        new CidEntry("아시아나항공(적립)", 1806212), new CidEntry("에어서울", 1800120)
    );
    ...
    public ResponseEntity<?> convert(@RequestBody Map<String, String> body) {
        ...
        List<CidEntry> cidList = buildCidList();           // CID 목록(고정 + 랜덤 5개)
    ...
    private LinkInfo fetchSequentially(String baseUrl, CidEntry entry) {
        ...
        modUrl = cleanCurrencyParameters(modUrl, currency);         // 통화 파라미터 정리
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                String actualCurrency = root.path("searchInfo")
                                             .path("currency")
                                             .asText(currency);
    ...
    private JsonNode fetchSecondaryDataJson(String hotelPageUrl, String currency) throws Exception {
        String pageUrl = addCurrencyToPageUrl(hotelPageUrl, currency);      // URL 한글화 + 통화 정리
        ...
        apiUrl = addCurrencyParameter(apiUrl, currency);                  // 중복 없이 통화 파라미터 추가
    ...
    private double extractPrice(JsonNode root) {
        ...
        if (p == 0) {
            ...
            if (!txt.isEmpty()) {
                ...
                if (!num.isEmpty()) try { p = Double.parseDouble(num); } catch (NumberFormatException ignored) { }
            }
        }
        return p;
    }
    ...
    /* ----------  통화 파라미터 중복 방지 & URL 한글화  ---------- */

    private String cleanCurrencyParameters(String url, String target) {
        try {
            ...
            for (String param : query.split("&")) {
                ...
                if (key.equals("currency")) continue;                       // currency 제거
                if (key.equals("currencyCode")) {
                    params.add(key + "=" + target);                         // currencyCode 교정
                } else {
                    params.add(param);
                }
            }
            String cleanQuery = String.join("&", params);
            return uri.getScheme() + "://" + uri.getHost() + uri.getPath() + "?" + cleanQuery;
        } catch (Exception e) {
            return url;
        }
    }
    ...
    private String addCurrencyParameter(String apiUrl, String currency) {
        if (apiUrl.contains("currencyCode=") || apiUrl.contains("currency=")) return apiUrl;
        return apiUrl + (apiUrl.contains("?") ? "&" : "?") + "currencyCode=" + currency;
    }
    ...
    private String addCurrencyToPageUrl(String url, String currency) {
        ...
        return cleanCurrencyParameters(url, currency);
    }
    ...
    private String buildKoreanCookies() {
        return String.join("; ",
            ...
            "cookieConsent=1",          "geolocation=KR",      "timezone=Asia%2FSeoul",
            "deviceType=desktop"
        );
    }
